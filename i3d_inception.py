...
#        x = AveragePooling3D((2, h, w), strides=(1, 1, 1), padding='valid', name='global_avg_pool')(x)
        ...
        x = Dropout(0.5)(x)
        x = Dense(12, activation='softmax', name='fc8')(x)
    ...
    if weights in WEIGHTS_NAME:
        if weights == WEIGHTS_NAME[0]:   # rgb_kinetics_only
            if include_top:
                weights_ur
    # load weightsl = WEIGHTS_PATH['rgb_kinetics_only']
...
#        downloaded_weights_path = get_file(model_name, weights_url, cache_subdir='models')
        downloaded_weights_path = 'rgb_inception_i3d_imagenet_and_kinetics_tf_dim_ordering_tf_kernels.h5'
        # downloaded_weights_path = 'flow_inception_i3d_imagenet_and_kinetics_tf_dim_ordering_tf_kernels_no_top.h5'
